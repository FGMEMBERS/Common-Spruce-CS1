...
setlistener("/sim/hud/visibility[1]", func {
    var view = getprop("/sim/current-view/view-number");
    if(view == nil or view == 0){
      setprop("/instrumentation/hud/show", getprop("/sim/hud/visibility[1]") or 0);
    }else{
      # so HUD is only selectable in the Cockpit
      setprop("/sim/hud/visibility[1]", 0);
    }
});

setlistener("/sim/current-view/view-number", func(n){
    if(n.getValue() == nil or n.getValue() == 0){
      setprop("/sim/hud/visibility[1]", getprop("/instrumentation/hud/show") or 0);
    }else{
      setprop("/sim/hud/visibility[1]", 0);
    }
});
...
setlistener("/sim/signals/fdm-initialized", func {
    ...
    setprop("/consumables/fuel/tank[3]/empty", 1);
    setprop("/controls/engines/engine[0]/mixture", 0);
    print("jettison tank[3] ... ok");
    #fire it up
    stewardessVoice();
    print("voice ... ok");
});
...
# Door action
var PilotDoor = func {
   var pilot = "/controls/doors/position-norm[0]";
   if (getprop(pilot) < 1){
      interpolate(pilot, 1  , 1);
   }else{
      interpolate(pilot, 0  , 1);
   }
}
var CopilotDoor = func {
   var copilot = "/controls/doors/position-norm[1]";
   if (getprop(copilot) < 1){
      interpolate(copilot, 1  , 1);
   }else{
      interpolate(copilot, 0  , 1);
   }
}


# For the steward/ess voice after landing
var stewardessVoice = func {
    #For the stewardess voice after landing
    var aft = getprop("position/altitude-agl-ft");
    var country = substr(getprop("sim/airport/closest-airport-id"),0,2);
    if (country == nil or country == "" or (country != "ED" and country != "LS")) country = "XY";
    #print(country);
    if(aft > 10){
      props.globals.getNode("/controls/airport/stewardess").setBoolValue(1);
      props.globals.getNode("/controls/airport/airport-country-id").setValue(country); 
    }
  settimer(stewardessVoice,12);
};
