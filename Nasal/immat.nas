...
var refresh_immat = func {
    ...
    for (var i = 10; i < 17; i += 1) {
	    if (i - 10 >= immat_size)
	        glyph = -1;
	    elsif (string.isupper(immat[i - 10]))
		    glyph = immat[i -10] - `A`;
	    elsif (string.isdigit(immat[i - 10]))
	        glyph = immat[i - 10] - `0` + 26;
	    else
	       glyph = 36;
...
var immat_dialog = gui.Dialog.new("/sim/gui/dialogs/cs1/status/dialog",
				  "Aircraft/Common-Spruce-CS1/Dialogs/immat.xml");
