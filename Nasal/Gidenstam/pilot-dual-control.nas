...
var connect = func (copilot) {
  ...
  var copilotCallsign = copilot.getChild("callsign").getValue();
  setprop("/sim/messages/copilot", copilotCallsign~" is on board, ... now.");
}
...
var main = {
  ...
  reset : func {
    if (me.active) {
      ...
      setprop("/sim/messages/copilot", "Copilot from board, ... now.");
